...
async function generateMosaic(mosaic) {
  ...
  for (let i = 0; i < svgs.length; i++) {
    try {
      const png = await svgToPng({ svgPath: path.resolve(projectPath, 'dist', svgs[i]), width: tileWidth, height: tileHeight })
      tiles.push(new Tile(await png.raw().toBuffer()))
    } catch(e) {
  ...
  while (tiles.length < vSize * hSize) {
    tiles = [...tiles, ...tiles.slice(0, tilesCount)]
  }
